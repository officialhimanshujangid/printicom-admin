...
export default function Tickets() {
  ...
  return (
    <div className={`tickets-container ${selectedTicket ? 'show-detail' : ''}`} style={{ height: 'calc(100vh - 100px)' }}>
      <div className="page-body-wrapper" style={{ height: '100%', gap: 0 }}>
        {/* Left List */}
        <div className="page-sidebar ticket-list-sidebar" style={{ display: 'flex', flexDirection: 'column', gap: 16, borderRight: '1px solid var(--border)', paddingRight: 20, height: '100%', width: 350 }}>
            ...
            {filteredTickets.map(t => (
              <div
                ...
                style={{
                  ...
                  border: selectedTicket?._id === t._id ? '1px solid var(--brand-primary)' : '1px solid var(--border)',
        ...
        {/* Right Detail View */}
        <div className="page-content ticket-detail-view" style={{ display: 'flex', flexDirection: 'column', height: '100%' }}>
          {selectedTicket ? (
            <div className="card" style={{ flex: 1, display: 'flex', flexDirection: 'column', padding: 0, overflow: 'hidden' }}>
              <div className="card-header" style={{ padding: 20, borderBottom: '1px solid var(--border)' }}>
                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', flexWrap: 'wrap', gap: 12 }}>
                  <div style={{ display: 'flex', alignItems: 'center', gap: 12 }}>
                    <button className="btn btn-ghost btn-icon mobile-only" onClick={() => setSelectedTicket(null)}>
                      <Clock size={18} style={{ transform: 'rotate(90deg)' }} />
                    </button>
                    <div>
                      <h2 style={{ fontSize: 20, fontWeight: 800, color: 'var(--text-primary)', marginBottom: 4 }}>{selectedTicket.subject}</h2>
                ...
                {(selectedTicket.responses || []).map((resp, i) => {
                  ...
                  return (
                    <div key={i} style={{ display: 'flex', gap: 16, flexDirection: isAdmin ? 'row-reverse' : 'row' }}>
                      <div className="avatar-placeholder" style={{ width: 40, height: 40, flexShrink: 0, background: isAdmin ? 'var(--brand-primary)' : 'var(--bg-highlight)' }}>
                        {isAdmin ? 'A' : selectedTicket.user?.name?.[0]}
                      </div>
                      <div style={{ flex: 1, maxWidth: '85%', background: isAdmin ? 'rgba(99,102,241,0.1)' : 'var(--bg-base)', padding: 16, borderRadius: isAdmin ? '12px 0 12px 12px' : '0 12px 12px 12px', border: '1px solid var(--border)' }}>
                        <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: 8 }}>
              ...
              <div style={{ padding: 16, borderTop: '1px solid var(--border)', background: 'var(--bg-base)' }}>
